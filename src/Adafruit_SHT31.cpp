...
bool Adafruit_SHT31::readTempHum(void) {
  ...
  if (readbuffer[2] != crc8(readbuffer, 2) ||
    ...
    return false;
  stemp_packet = (uint16_t)(readbuffer[0] << 8) | readbuffer[1];
  shum_packet = (uint16_t)(readbuffer[3] << 8) | readbuffer[4];
...
uint16_t Adafruit_SHT31::readTemperaturePacket() {
  return stemp_packet;
}
uint16_t Adafruit_SHT31::readHumidityPacket() {
  return shum_packet;
}

void Adafruit_SHT31::update() {
  readTempHum();
}
